...
def health_check():
    return jsonify({
        ...
        "service": "Free Fire Bot API",
        "timestamp": str(datetime.now())
    })

def run_flask():
    # Render uses PORT env var, fallback to 10000 for local testing
...
async def MaiiiinE():
    ...
    # Flask is now started BEFORE this point, not after
...
if __name__ == '__main__':
    # CRITICAL FIX: Start Flask IMMEDIATELY in a separate thread
    # This ensures Render detects the port binding within 60 seconds
    flask_thread = threading.Thread(target=run_flask, daemon=True)
    flask_thread.start()
    
    # Give Flask a moment to start binding
    import time
    time.sleep(2)
    print("✅ Flask server started, now connecting bot...")
